...
export function ServerInterface() {
  ...
  const getTableStatus = (table: DiningTable) => {
    ...
    if (table.status && hasActiveOrder) {
      return {
        status: "occupied",
        label: "Occupied",
  ...
  const availableTables = tables.filter((table) => !table.status);
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 mb-3 sm:mb-4">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs sm:text-sm text-muted-foreground mt-1 hidden sm:block">
                Take orders for your tables • Provide excellent service
              </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4">
            {filteredProducts.map((product) => {
              ...
              return (
                <Card
                  key={product.id}
                  className="hover:shadow-md active:scale-95 transition-all duration-150 touch-manipulation"
                >
                  <CardHeader className="pb-2 sm:pb-3 p-3 sm:p-6">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        <CardTitle className="text-sm sm:text-base lg:text-lg leading-tight truncate">
                          {product.name}
                        </CardTitle>
                        {product.description && (
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 max-h-40 sm:max-h-48 overflow-y-auto">
                {tablesWithStatus.map((table) => {
                  const canSelect =
                    table.statusInfo.status === "available" ||
                    table.statusInfo.status === "seated";
                  return (
                    <Button
                      key={table.id}
                      ...
                      <div
                        className={`absolute -top-1 -right-1 w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full ${
                          table.statusInfo.status === "available"
                            ...
                            : table.statusInfo.status === "seated"
                              ? "bg-blue-500"
                              : table.statusInfo.status === "pending"
                                ? "bg-yellow-500"
                                : "bg-red-500"
                        }`}
                      />
          ...
          {selectedTable && (
            <div className="mt-3 p-2 sm:p-3 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <div className="text-[10px] sm:text-xs text-blue-700">
                ...
                {selectedTable.location || "Main floor"}
              </div>
            </div>
          )}
        </div>
        ...
        {cart.length > 0 ? (
          ...
          <div className="p-3 sm:p-4 border-t border-border bg-card flex-shrink-0">
            <div className="text-center text-muted-foreground">
              ...
              <p className="text-xs sm:text-sm mt-1">
                {selectedTable
                  ? "Add items to start taking an order"
